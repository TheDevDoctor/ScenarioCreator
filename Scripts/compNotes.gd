...
func add_differential(button):
	...
	$"../../../..".bedDifferentials.append(differential)
...
func remove_diff(diff):
	$"../../../..".bedDifferentials.erase(diff.get_name())
	create_diff_buttons([diff.get_name()], true)
	diff.free()
	print(differentials)

func set_notes():
	if get_node("../../../..").bedNotes != null:
		$notes.set_text($"../../../..".bedNotes)
	else:
		$notes.set_text("")
	add_diifs_to_labels()

func save_notes():
	$"../../../..".bedNotes = $notes.get_text()
