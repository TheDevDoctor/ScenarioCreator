...
func talkToServer(url, mode, data):
	...
	if HTTP.has_response():
		...
		while HTTP.get_status() == HTTPClient.STATUS_BODY:
			...
			if mode == 'bot':
				print(str(RESULT))
				get_node("/root/MainFrame/Current/History/MainPatch/TestScreen/pixelBotBox").return_string_from_bot(RESULT)
			elif mode == 'game':
				print(str(RESULT))
				get_node("/root/MainFrame/Current/TestScreen/ScenarioDetails/ViewportContainer/Viewport/World/GUI/pixelBotBox").return_string_from_bot(RESULT)
