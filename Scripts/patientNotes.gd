...
func _ready():
	...
	set_screen(0)
	diffsDict = singleton.load_file_as_JSON("res://JSON_files/conditions.json")
...
func hide_notes():
	$NinePatchRect.set_position(Vector2(-560, 2))
...
func set_bed(bed):
	...
	currBed = bed
	if $"../..".bedNotes != null:
		$NinePatchRect/NinePatchRect2/TextEdit.set_text($"../..".bedNotes)
	if $"../..".bedDifferentials.size() > 0:
		differentials = $"../..".bedDifferentials
	add_diifs_to_labels()
...
func done_pressed():
	...
	set_screen_modulates(0)
	add_diifs_to_labels()
	$"../..".bedDifferentials = differentials
...
func save_notes():
	$"../..".bedNotes = $NinePatchRect/NinePatchRect2/TextEdit.get_text()
