...
func _ready():
	...
	#set previous data:
	prescriptionDict = get_node("../..").bedPrescriptions
...
func print_data():
	var bed = get_node("/root/MainFrame").scenarioDict
#	var bedsArray = ["BED1"]
##	for bed in beds:
...
#		if beds[bed] != null:
	...
	var presentingComplaint = bed["PatientDetails"]["PresentingComplaint"]
...
#		get_node("/root/singleton").load_file_as_JSON("res://JSON_files/investigations.json", data)
		...
		for ix in get_node("../..").bedInvestigations["Pending"]:
			print_ix(ix)
		...
		get_node(screen + "/Title").set_text(bed)
		storyIxChange = get_node("/root/MainFrame").scenarioDict["Investigations"]
...
#	patch.set_texture(load("res://Graphics/baseDataContainer.png"))
#	patch.set_name(diff)
...
#	cancelButton.texture_normal = load("res://Graphics/DeleteButton.png")
#	cancelButton.texture_hover = load("res://Graphics/DeleteButtonHover.png")
...
func print_investigations_for_bed(bed):
	for child in get_node("InfoScreen/InvestigationsBg/ScrollContainer/VBoxContainer").get_children():
		child.free()
	var complete = get_node("../..").bedInvestigations["Complete"]
	var pending = get_node("../..").bedInvestigations["Pending"]
	...
	if pending.size() > 0 and complete.size() > 0:
		...
		separator.set_texture(preload("res://Graphics/CompSplitter.png"))
	...
	for ix in pending:
		var node = load("res://Scenes/IxOrderButton.tscn").instance()
		node.set_name(ix)
...
func return_ixComplete_texture_button(ix):
	...
	button.set_normal_texture(preload("res://Graphics/investigationReturnedNormal.png"))
	button.set_hover_texture(preload("res://Graphics/ixButtonHover.png"))
	button.set_pressed_texture(preload("res://Graphics/ixButtonPressed.png"))
	button.set_name(ix)
...
func open_side_screen(btn):
	if btn.is_pressed() == true:
		...
		$InfoScreen/ViewIx/IxBg/ImagingContainer/sideScreenButton.margin_right = -167
		$InfoScreen/ViewIx/IxBg/ImagingContainer/sideScreenButton.margin_left = -200
	else:
		$InfoScreen/ViewIx/IxBg/ImagingContainer/SideScreen.hide()
		$InfoScreen/ViewIx/IxBg/ImagingContainer/sideScreenButton.margin_right = 4
		$InfoScreen/ViewIx/IxBg/ImagingContainer/sideScreenButton.margin_left = -29
...
func order_ix_pressed():
	...
	for ix in ordered[currentBed]:
		if get_node("../..").bedInvestigations["Complete"].has(ix) == false:
			get_node("../..").bedInvestigations["Complete"].append(ix)
...
func drug_info_pressed(med):
	open_info(data[presType][med]["Info"], med)
	...
	if presType == "Medications":
		...
		for child in children:
			if activeType.has(child.get_name()):
				prescriptionDict[child.get_name()][currentMed] = {}
				for node in child.get_children():
					if node.get_class() == "OptionButton":
						prescriptionDict[child.get_name()][currentMed][node.get_name()] = node.get_item_text(node.get_selected())
	elif presType == "Fluids":
		...
		prescriptionDict["Infusion"][currentMed + drugsString] = {}
		for node in children:
			if node.get_class() == "OptionButton":
				prescriptionDict["Infusion"][currentMed  + drugsString][node.get_name()] = node.get_item_text(node.get_selected())
		if infusionDrugsAdded.size() > 0:
			prescriptionDict["Infusion"][currentMed  + drugsString]["DrugsAdded"] = {}
			for drug in infusionDrugArray:
				prescriptionDict["Infusion"][currentMed  + drugsString]["DrugsAdded"][drug.get_name()] = {"Dose": drug.get_node("Dose").get_item_text(drug.get_node("Dose").get_selected()), "Unit": drug.get_node("Unit").get_item_text(drug.get_node("Unit").get_selected())}
	elif presType == "Oxygen":
		var children = get_node("PrescribeScreen/PresDetails/OxygenAddScreen/Oxygen").get_children()
		prescriptionDict["Oxygen"][currentMed] = {}
		for node in children:
			if node.get_class() == "OptionButton":
				prescriptionDict["Oxygen"][currentMed][node.get_name()] = node.get_item_text(node.get_selected())
...
func print_prescribed_list():
	...
	for type in prescriptionDict:
		if prescriptionDict[type].size() > 0:
			...
			for med in prescriptionDict[type]:
				...
				for detail in printOrder[type]:
					...
					if detail != "DrugsAdded":
						if detail == "Doses":
							detailCont.get_node("detailText").set_text(prescriptionDict[type][med][detail] + " " + prescriptionDict[type][med]["Units"])
						elif detail == "Duration":
							detailCont.get_node("detailText").set_text(prescriptionDict[type][med][detail] + " " + prescriptionDict[type][med]["DurUnits"])
						elif detail == "Volume":
							detailCont.get_node("detailText").set_text(prescriptionDict[type][med][detail] + " " + prescriptionDict[type][med]["VolUnits"])
						elif detail == "RunTime":
							detailCont.get_node("detailText").set_text(prescriptionDict[type][med][detail] + " " + prescriptionDict[type][med]["RunTimeUnits"])
						else:
							detailCont.get_node("detailText").set_text(prescriptionDict[type][med][detail])
						node.get_node("DetailsContainer").add_child(detailCont)
					
					elif detail == "DrugsAdded":
						node.get_node("DrugsAddedContainer").show()
						for drug in prescriptionDict[type][med][detail]:
							var detailCont2 = return_detail_frame_node(detail)
							detailCont2.get_node("detailText").set_text("+ " + drug + " " + prescriptionDict[type][med][detail][drug]["Dose"] + prescriptionDict[type][med][detail][drug]["Unit"])
...
func return_detail_frame_node(detail):
	var detailsCont = NinePatchRect.new()
	detailsCont.set_texture(preload("res://Graphics/investigationOrderNormal.png"))
	detailsCont.set_h_size_flags(3)
	detailsCont.set_patch_margin(MARGIN_LEFT, 5)
	detailsCont.set_patch_margin(MARGIN_TOP, 5)
	detailsCont.set_patch_margin(MARGIN_BOTTOM, 5)
...
func remove_prescription(node):
	...
	prescriptionDict[type].erase(node.get_name())
	node.free()
	print_prescribed_list()
	if currScreen == "PrescribeScreen":
...
func stored_pres_btn_pressed(btn):
	if btn.get_name() == "Clear":
		...
		for type in prescriptionDict:
			prescriptionDict[type].clear()
	elif btn.get_name() == "Order":
		get_node("../..").bedPrescriptions = prescriptionDict
		open_screens("Beds", null)
		currentBed = null
